...
class Admin_base extends CI_Controller 
{
	...
	//-----------------------------載入參數-------------------------------
		...
		$header_output['page_meun'] ="service_base";
		...
		switch($page)
		{
			...
			case 'update':
				...
				$page_output['base_data'] = $register[0];


				break;

			case 'insert':
				$page_output['base_data'] 	= '';
				break;
			default:
				break;
		}
		...
		$this->load->view('admin/base/'.$page,$page_output);
...
// 									);
// 				//---------------------載入參數的資料-----------------------------
// 				$table 	= array('service_base');					//各個資料表
// 				$data	= array($post);						//各個資料表 - 修改資料
// 				$this->Common->insert($table, $data);		//送出更新	

// 		//----------------轉換function-------------------
// 		redirect('admin_base/index/index', 'refresh');
