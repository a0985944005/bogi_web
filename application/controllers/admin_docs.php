<?php
class Admin_docs extends CI_Controller 
{
	...
	//-----------------------------載入參數-------------------------------
		...
		switch($page)
		{
			...
			case 'update':
				...
				$page_output['docs_data'] = $register[0];


				break;

			case 'insert':
				$page_output['docs_data'] 	= '';
				break;
			default:
				break;
		}
		...
		$this->load->view('admin/docs/'.$page,$page_output);
...
public function insert($blog_id=FALSE)
	{
		...
		// echo $_FILES['pic']['name'][0];
		// exit;
		if (!empty($_FILES['pic']['name'][0]))
			 {
				$config = array(
					'upload_path'   => 'application/views/front/document/',
					'allowed_types' => '*',
					'encrypt_name'  => TRUE,                       
				);
		...
		//----------------轉換function-------------------
		redirect('admin_docs/index/index', 'refresh');
  ...
  //------------------刪除部落格項目------------------------------
	...
	public function download_files($download_id)
	{
		...
		$post 	= array(//輸入的select相關資訊
			...
			$docs_type_data = $this->Common->select($post);	
		
		//echo $blog_type_data[0]['file_name'];
				
		$this->load->helper('download');		
		$data = file_get_contents("application/views/front/document/".$docs_type_data[0]['file_name']); // Read the file's contents
		$name = $docs_type_data[0]['file_name'];
	...
	public function delete_files($delete_id)
	{
		...
		$file = "application/views/front/document/".$blog_type_data[0]['file_name'];
		@unlink($file);
