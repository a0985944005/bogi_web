...
public function insert($blog_id=FALSE)
	{
		...
		if (!empty($_FILES['pic']['name'][0])) {
				$config = array(
					'upload_path'   => 'application/views/news_img/',
					'allowed_types' => '*',
					'encrypt_name'  => TRUE,                       
				);
			...
			//---------------------寫入的資料-----------------------------
				$post		= array (//由view insert表單轉送過來的資料
									...
									'news_pic'	=>	$Mutil_upload_msg[0]['orig_name'],
									'news_img'	=>	$Mutil_upload_msg[0]['file_name'],
...
//---------------------檔案上傳-----------------------------
  ...
  //------------------刪除部落格項目------------------------------
	...
	public function download_files($delete_id)
	{
		...
		$data = file_get_contents("application/views/news_img/".$news_type_data[0]['news_img']); // Read the file's contents
		$name = $news_type_data[0]['news_pic'];
	...
	public function delete_files($delete_id)
	{
		...
		$file = "application/views/news_img/".$blog_type_data[0]['news_img'];
		@unlink($file);
